...
function codelove(){
    ...
    var textAnimate = eval(Jscex.compile("async", function () {
        ...
        together.setFullYear(2016, 7-1, 15);            
    ...
    var runAsync = eval(Jscex.compile("async", function () {
        $await(seedAnimate()); //ra cái trái tim bắt click vô
        $await(growAnimate()); //vẽ ra cái gốc cây =))
        $await(flowAnimate()); //hiện ra 1 đống hoa trái tim :v
        $await(moveAnimate()); //dịch chuyển cái cây qua bên phải
        textAnimate().start(); //đếm ngày giờ
        $await(jumpAnimate()); // lá trái tim rơi
    }));
